#!/usr/bin/env python3
"""
Análise Visual com Mistral Pixtral
==================================
Usa modelo Pixtral-12B para análise de imagens de imóveis
"""
        ...
        with open(env_path, "r") as f:
            for line in f:
                if line.startswith("MISTRAL_API_KEY="):
                    MISTRAL_API_KEY = line.strip().split("=", 1)[1]
                    break
...
def analyze_image_with_mistral(image_path: str) -> dict:
    """
    Analisa uma imagem usando Mistral Pixtral
    """
...

{
  ...
  "observacoes": "<breve descrição do que é visível>"
}
...
def analyze_batch_mistral(base_dir: str = ".tmp/batch_resultado"):
    """
    ...
    print(f"🤖 Analisando {len(property_dirs)} imóveis com Mistral Pixtral...\n")
    ...
    for prop_dir in property_dirs:
        ...
        if "visual_analysis_mistral" in data:
        ...
        print(f"  📊 Analisando {len(images)} imagens com Pixtral...")
        ...
        for img_info in images:
            ...
            try:
                analysis = analyze_image_with_mistral(str(img_path))
                if analysis:
        ...
        if analyses:
            ...
            data["visual_analysis_mistral"] = visual_analysis
...
if __name__ == "__main__":
    ...
    base_dir = sys.argv[1] if len(sys.argv) > 1 else ".tmp/batch_resultado"
    analyze_batch_mistral(base_dir)
